...
| **POST** | `/api/v1/admins/organizations` | **Request body.** **Response `201`** includes a generated `organization_id` — put that value on each student Cognito user as `custom:organization_id`. <pre>{<br>  "name": "IEEE Mapua"<br>}</pre>**Response:** <pre>{<br>  "data": {<br>    "organization_id": "3f2c1a90-....",<br>    "name": "IEEE Mapua"<br>  }<br>}</pre> |
...
| **POST** | `/api/v1/admins/signatories` | **Request body.** Writes GSI4 `ROLE#{ROLE}#ORG#{org}`. **Response `201`**. Put `signatory_id` on the Cognito user as `custom:signatory_id`. <pre>{<br>  "name": "Prof. Juan Dela Cruz",<br>  "role": "adviser",<br>  "organization_id": "a1b2"<br>}</pre> |
